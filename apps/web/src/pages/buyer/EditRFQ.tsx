...
export default function EditRFQ() {
  ...
  const { data: rfq, isLoading: isLoadingRfq } = useQuery({
  ...
  useEffect(() => {
    if (rfq) {
      reset({
        ...
        purchaseFrequency: rfq.purchaseFrequency || undefined,
        budgetMin: rfq.budgetMin || undefined,
        budgetMax: rfq.budgetMax || undefined,
        budgetCurrency: rfq.budgetCurrency || CurrencyCode.CNY,
        preferredRegions: Array.isArray(rfq.preferredRegions)
          ? rfq.preferredRegions.join(', ')
          : rfq.preferredRegions || '',
        deliveryDeadline: rfq.deliveryDeadline
          ? new Date(rfq.deliveryDeadline).toISOString().split('T')[0]
          : '',
        validUntil: rfq.validUntil
          ? new Date(rfq.validUntil).toISOString().split('T')[0]
          : '',
        isPublic: rfq.isPublic ?? true,
        showCompanyInfo: rfq.showCompanyInfo ?? true,
        status: rfq.status || RFQStatus.DRAFT,
      });
    }
  }, [rfq, reset]);
  ...
  const updateMutation = useMutation({
    mutationFn: (data: RFQFormData) => {
      ...
      return marketplaceRfqApi.update(id!, payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['my-rfqs'] });
      queryClient.invalidateQueries({ queryKey: ['marketplace-rfq', id] });
      toast.success(t('buyer.rfqUpdated', 'RFQ updated successfully'));
      navigate('/buyer/rfqs');
    },
    onError: (error: unknown) => {
      ...
      if (Array.isArray(msg)) {
        ...
        toast.error(msg || t('buyer.rfqUpdateFailed', 'Failed to update RFQ'));
      }
    },
  });
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-6">
        <div className="text-center py-12">
          <p className="text-gray-500">{t('buyer.rfqNotFound', 'RFQ not found')}</p>
          <button
    ...
    <div className="max-w-3xl mx-auto px-4 py-6">
      ...
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">
          {t('buyer.editRfq', 'Edit RFQ')}
        </h1>
        <p className="text-gray-600 mt-1">
          {t('buyer.editRfqDesc', 'Update your purchase requirement')}
        </p>
      </div>
          ...
          <div className="mt-4 pt-4 border-t border-gray-200">
            ...
            <div className="flex flex-wrap gap-4">
              <label className="flex items-center gap-2">
                ...
                />
                <span className="text-sm text-gray-700">{t('buyer.draft', 'Draft')}</span>
              </label>
              <label className="flex items-center gap-2">
                <input
                  type="radio"
                  ...
                  className="w-4 h-4 text-purple-600 border-gray-300 focus:ring-purple-500"
                />
                <span className="text-sm text-gray-700">{t('buyer.open', 'Open')}</span>
              </label>
              <label className="flex items-center gap-2">
                <input
                  type="radio"
                  ...
                  className="w-4 h-4 text-purple-600 border-gray-300 focus:ring-purple-500"
                />
                <span className="text-sm text-gray-700">{t('buyer.closed', 'Closed')}</span>
              </label>
            </div>
          </div>
        </div>
